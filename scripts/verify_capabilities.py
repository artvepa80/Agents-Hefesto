...
def validate_devops_analyzers(manifest):
    ...
    discovered_mods = {
        m.name for m in pkgutil.iter_modules(devops_pkg.__path__) if m.name.endswith("_analyzer")
    }

    # Check consistency count
    if len(declared_devops) != declared_count:
        errors.append(
            f"Manifest Inconsistency: declared_count={declared_count} but found {len(declared_devops)} items"
        )

    if len(discovered_mods) != declared_count:
        errors.append(
            f"Code/Manifest Mismatch: Code has {len(discovered_mods)} analyzers, Manifest declares {declared_count}"
        )
        print(f"   (Code found: {sorted(list(discovered_mods))})")

        # Diff reporting
        declared_names = {
            item.get("analyzer_module", "").split(".")[-1]
            for item in declared_devops
            if item.get("analyzer_module")
        }
...
def validate_code_languages(manifest):
    ...
    if items_count != declared_count:
        errors.append(
            f"Code Languages Inconsistency: declared_count={declared_count} but found {items_count} items"
        )
